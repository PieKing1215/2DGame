...
public class BasicEnemy extends Mob {
	...
	public BasicEnemy(Level level, int x, int y, int damage, int hp, int viewDist, boolean shouldDrop, int id) {
		super(level, "BasicEnemy", x, y, 1);
		this.damage = damage;
		this.hp = hp;
	...
	public void tick() {
        ...
        if(getLoc().getDistance(player.getLoc())<viewDist) {
        	//System.out.println("X: "+x+" "+xa2);
        	//System.out.println("Y: "+y+" "+ya2);
        if(getLoc().getDistance(player.getLoc())<9) {
        	if(animCount >= 80) {
        	if(Player.damage(damage)){
        		player.move(-Math.round((float)(0.5*(x-xa2))), -Math.round((float)(0.5*(y-ya2))));
        		Random rand = Rand.getRand();
        ...
        } else if((speed2 == 2&&id==0)||(speed2 == 4&&id==1)||(speed2 == 4&&id==2)){
        	speed2=0;
        	 if(x>xa2){
             	xa--;
             } 
        ...
        if(Player.spacePress) {
        	if(getLoc().getDistance(player.getLoc())<12) { 
        		move((x-xa2), (y-ya2));
        		damage(1);
        	}
        }
        ...
        try{if(Tile.getById(Level.tiles[((int) Math.floor(((x-4) +  (double)8 / 2) / 8))+((int) Math.floor(((y) +  (double)8 / 2) / 8)) *Level.width]).isSolid()){
        	x++;
        }
